...
Each course should copy `C:\bot\template_v9` into a new course directory and work directly in that course directory.
...
The card stays hidden unless the matching `code_m##.js` file exists and defines usable examples. When visible, the code example card renders in the scrollable slide content above the fixed narration dock. The code asset should define `window.COURSE_CODE_MODULE`:
...
The narration player is a fixed, rectangular bottom dock inside the SCORM iframe. It remains visible while the learner scrolls and includes the audio timeline plus persistent controls for captions, narration text, playback speed, header visibility, image visibility, and code example visibility. These preferences are stored per course. `audio-controls.js` measures the dock with `ResizeObserver`, and `base.css` reserves matching bottom space so slide content and the status footer are not obscured.

The `Hide Header` button visually hides the `.title-card` on instructional modules and stores that preference for the course, similar to `CC Off` and `Hide Narration`. The header keeps its existing semantic markup, but the v9 stylesheet renders it as plain centered title text with no card background, border, padding, or shadow. The title remains available to assistive technology.
...
Keep the header visible by default. A visible title/header is not a separate SCORM requirement, but it supports accessibility by giving each slide a clear page heading. Do not add the course title above the module title. LMS connection and progress text belongs in the centered `.slide-status-footer` at the bottom of the slide, outside every card. If a course is changed to hide headers by default, preserve equivalent orientation elsewhere, such as a visible current module label, and keep an accessible heading available to assistive technology.

Module graphics use the browser Fullscreen API when activated, allowing the image to fill the entire browser viewport even when the SCO is hosted in a fullscreen-enabled LMS iframe. Activating the image again collapses it, and Escape also exits fullscreen. If the LMS or browser denies fullscreen, the runtime retains the iframe-sized overlay as a fallback.

## Enterprise LMS Presentation

The default v9 presentation uses a light enterprise LMS palette: light-gray workspace, white surfaces, navy/slate text, muted blue accents, subtle borders, and restrained shadows. Slide headings use the operating system's enterprise UI font stack, including Segoe UI on Windows. Keep code blocks dark for code readability, but do not restore the old dark/glow theme to the course shell.
...
Every SCO document must call `LMSInitialize()` even when the shared LMS adapter already appears usable. Moodle updates its internal current-SCO identifier during that call; skipping it can route later modules into the previous SCO's tracking record. Moodle may return error `101` after selecting the newly requested SCO when a prior document has not finished, so the wrapper validates the resulting session before treating that case as fatal. Moodle adapters that report a false mutation result with `LMSGetLastError() === "0"` are treated as successful writes.
...
The package root must include these SCORM 1.2 schemas copied from the v9 template:
...
## Updating Older Courses To v9

Older generated courses are usually close to v9 in content, but not in structure. They often already have module HTML files, images, MP3 narration, VTT captions, visible titles, narration text, and a final quiz. The problem is that earlier templates and AI drift may have placed those items under different element names, card layouts, inline scripts, data attributes, or helper JavaScript blocks.

Treat migration as extraction and rebuild, not as a markup patch. Copy `C:\bot\template_v9` into a new upgraded course folder, then move normalized course content into the v9 asset pattern. This is usually safer than editing the old module files in place because v9 expects a stable runtime, stable script order, stable asset names, LMS-owned multi-SCO navigation, and checked persistence results.
...
3. Copy C:\bot\template_v9 into a new target directory.
4. Generate fresh m01.html, m02.html, etc. from v9 base.html and quiz.html.
...
Manifest cleanup is part of the migration. Rebuild `imsmanifest.xml` as multi-SCO, with one organization item and one uniquely identified launchable resource per module. Each item must reference its matching resource, and each resource must launch its matching `m##.html`. The LMS owns navigation between those SCOs; retain the v9 preview-only link guards and per-SCO state handling. Include `index.html` only as a real helper file if it exists; never use it as a SCO launch target. List the real runtime, CSS, and asset files used by each module. Do not list old files, missing files, source-only files, or nonexistent final quiz media. Copy the four v9 SCORM 1.2 XSDs to the package root and retain the full `xsi:schemaLocation` mapping.
...
## Sample Prompt For Batch Upgrading Courses To v9

Use this when a directory contains multiple older generated courses and you want Codex to upgrade all of them to the current v9 template without manually migrating each course:

```text
Upgrade all SCORM-style courses in [SOURCE_DIRECTORY] to the current v9 template.

Template:
C:\bot\template_v9

Output:
For each course found in [SOURCE_DIRECTORY], create an upgraded v9 course next to the original course using this naming pattern:
<original_course_folder_name>_v9
...
- Copy C:\bot\template_v9 to the new upgraded course folder.
- Rebuild module HTML files from the v9 template files instead of patching old drifted HTML in place.
...
Use the v9 modular asset pattern:
...
Then make a complete prompt for Codex that builds the course using the v9 template.
...
- Copy C:\bot\template_v9 to the target course directory.
...
Then make a complete prompt for Codex that builds the course using the v9 template. No fictional scenarios in the content.
