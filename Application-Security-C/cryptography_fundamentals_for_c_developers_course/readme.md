...
The audio player card includes persistent controls for captions, narration text, playback speed, and header visibility. The `Hide Header` button visually hides the `.title-card` on instructional modules and stores that preference for the course, similar to `CC Off` and `Hide Narration`. The title card remains available to assistive technology.
...
2. Identify the course title, module order, module titles, narration text, image files, MP3 files, VTT files, and quiz questions.
