(function () {
  ...
  function mediaSource(audioBase, extension) {
    var source = String(audioBase || "").trim().replace(/\\/g, "/");
    source = source.replace(/\.(mp3|vtt)$/i, "");
    if (!source) return "";
    if (source.indexOf("/") === -1) source = "course_assets/" + source;
  ...
  function initialize() {
    ...
    audio.src = mediaSource(audioBase, ".mp3");

    var trackElement = document.createElement("track");
    trackElement.kind = "captions";
    trackElement.label = "English";
    trackElement.srclang = "en";
    trackElement.src = mediaSource(audioBase, ".vtt");
