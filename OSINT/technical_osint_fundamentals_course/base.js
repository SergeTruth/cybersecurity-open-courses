(function () {
  ...
  function highlightOsintCode() {
    var shellKeywords = {
      "case": true, "do": true, "done": true, "else": true, "esac": true, "fi": true,
      "for": true, "function": true, "if": true, "in": true, "then": true, "while": true
    };
    var shellCommands = {
      "cd": true, "cp": true, "curl": true, "date": true, "dig": true, "mkdir": true,
      "printf": true, "rg": true, "sha256sum": true, "test": true
    };
    var pythonKeywords = {
      "False": true, "None": true, "True": true, "and": true, "as": true, "class": true,
      "def": true, "elif": true, "else": true, "for": true, "from": true, "if": true,
      "import": true, "in": true, "is": true, "not": true, "or": true, "return": true,
      "set": true, "with": true
    };
    var pythonBuiltins = {
      "any": true, "csv": true, "datetime": true, "dict": true, "encoding": true,
      "errors": true, "frozen": true, "int": true, "ipaddress": true, "json": true,
      "list": true, "open": true, "print": true, "re": true, "set": true, "sorted": true,
      "str": true, "sum": true, "timezone": true, "tuple": true
    };
    ...
    Array.prototype.forEach.call(blocks, function (block) {
      ...
      while (index < source.length) {
        ...
        if ((language === "shell" || language === "bash") && character === "$") {
          var varEnd = index + 1;
          if (source.charAt(varEnd) === "{") {
            var close = source.indexOf("}", varEnd + 1);
            varEnd = close === -1 ? varEnd + 1 : close + 1;
          } else if (source.charAt(varEnd) === "(") {
            var commandClose = source.indexOf(")", varEnd + 1);
            varEnd = commandClose === -1 ? varEnd + 1 : commandClose + 1;
          } else {
        ...
        if ((language === "shell" || language === "bash") && character === "-" && /[A-Za-z-]/.test(source.charAt(index + 1))) {
        ...
        if (/[A-Za-z_]/.test(character)) {
          ...
          } else if ((language === "shell" || language === "bash") && shellKeywords[word]) {
            output += codeToken(word, "tok-keyword");
          } else if ((language === "shell" || language === "bash") && shellCommands[word]) {
